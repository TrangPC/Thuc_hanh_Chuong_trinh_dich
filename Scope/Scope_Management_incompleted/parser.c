...
//       eat(SB_RPAR);
//     } else {
//       // If it's not followed by '(', it means it's just an identifier, so we don't expect arguments.
//       error(ERR_INVALID_RETURN, currentToken->lineNo, currentToken->colNo);
//     }
//      if (var->funcAttrs->returnType == NULL) {
//         // Function has no return type (void)
//         error(ERR_INVALID_RETURN, currentToken->lineNo, currentToken->colNo);
//       } else {
//         // If the function has a return type, but we encounter a RETURN here, it's invalid.
//         error(ERR_INVALID_RETURN, currentToken->lineNo, currentToken->colNo);
//       }
//     break;   
// default: 
  ...
  switch (var->kind) {
    ...
    case OBJ_FUNCTION:
      ...
      if (lookAhead->tokenType == SB_LPAR) {
        ...
        // If it's not followed by '(', it means it's just an identifier, so we don't expect arguments.
        if (var->funcAttrs->returnType != NULL) {
          // Function has a return type, but RETURN is missing
          error(ERR_INVALID_RETURN, currentToken->lineNo, currentToken->colNo);
        }
      }
